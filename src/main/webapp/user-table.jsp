...
<link rel="stylesheet" href='<c:url value="/https://cdn.datatables.net/1.10.19/css/jquery.dataTables.min.css"></c:url>'>
<!-- animation CSS -->
...
<body>
			...
			<div class="container-fluid">
				<div class="row bg-title">
					...
					<div class="col-lg-9 col-sm-8 col-md-8 col-xs-12 text-right">
						<a href='<c:url value="/user/add"></c:url>'
									...
									<tbody>
										<c:forEach items="${listUsers}" var="user" varStatus="loop">
											<tr>
												...
												<td>${user.getFullname()}
												<td>${user.email}</td>
												<td>${user.getRoleNameFromRoleID()}</td>
												<td><a href='<c:url value="/user/update?id=${user.id}"></c:url>' class="btn btn-sm btn-primary">Sửa</a>
													<%--<a href='<c:url value="/user/delete?id=${user.id}"></c:url>' class="btn btn-sm btn-primary">Xóa</a> --%>
													
													<%-- thủ thuật custom attribute: tự tạo attribute user-id --%>
													<a href="#" user-id="${user.id}" class="btn btn-sm btn-danger btn-delete-user">Xóa</a> 
													<a href='<c:url value="/user/view?id=${user.id}"></c:url>' class="btn btn-sm btn-info">Xem</a></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
	...
	<script src='<c:url value="/js/user-table.js?version=1"></c:url>'></script>
	<script>
