...
<body>
			...
			<div class="container-fluid">
				<div class="row bg-title">
					...
					<div class="col-lg-9 col-sm-8 col-md-8 col-xs-12 text-right">
						<a href='<c:url value="/task/add"></c:url>' class="btn btn-sm btn-success">Thêm
							mới</a>
					</div>
					<!-- /.col-lg-12 -->
				</div>
									...
									<tbody>
										<c:forEach items="${listTasks}" var="task" varStatus="loop">
											<tr>
												...
												<td>${task.getNameOfUserID()}</td> 
												<td>${task.start_date}</td>
												<td>${task.end_date}</td>  
												<td>${task.getNameOfStatusID()}</td> 
												<td><a href='<c:url value="/task/update?task_id=${task.id}"></c:url>' class="btn btn-sm btn-primary">Sửa</a>
													<a href="#" task-id="${task.id}" class="btn btn-sm btn-danger btn-delete-task">Xóa</a> 
													</td>
											</tr>
										</c:forEach>
									</tbody>
	...
	<script
		src='<c:url value="/https://cdn.datatables.net/1.10.19/js/jquery.dataTables.min.js"></c:url>'></script>
	<!--Wave Effects -->
	<script src='<c:url value="/js/waves.js"></c:url>'></script>
	<!-- Custom Theme JavaScript -->
	<script src='<c:url value="/js/custom.min.js"></c:url>'></script>
	<script src='<c:url value="/js/task-table.js?version=1"></c:url>'></script>
	<script>
